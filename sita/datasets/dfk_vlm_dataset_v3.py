"""DFK Vision-Language Dataset V3 — social media content analysis.
...
By default uses pre-split files (train_aug.csv / val_aug.csv).  Optionally
...
logger = logging.getLogger("sita.datasets.dfk_vlm_dataset_v3")
...
@DATASET_REGISTRY.register("dfk_vlm_dataset_v3")
class DFKVLMDatasetV3(BaseDatasetLoader):
    """Load the DFK vision dataset (V3) from a local directory.
    ...
    **Split modes** (controlled by ``use_fixed_splits``):
    ...
    1. *Fixed splits* (default): reads ``train_aug.csv`` and ``val_aug.csv`` from
